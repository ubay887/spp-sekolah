...
vue__WEBPACK_IMPORTED_MODULE_0___default.a.use(_trevoreyre_autocomplete_vue__WEBPACK_IMPORTED_MODULE_2__["default"]);
...
new vue__WEBPACK_IMPORTED_MODULE_0___default.a({
  ...
  watch: {
    'tmp.harga': function tmpHarga() {
      if (parseInt(this.tmp.harga) <= 10000) {
        ...
        this.showError = true;
        this.msgError = "Jumlah bayar tidak boleh lebih dari Rp. ".concat(this.tmp.sisa);
      } else {
  ...
  methods: {
    search: function search(input) {
      var url = "../getSiswa?keyword=".concat(input);
      return new Promise(function (resolve) {
        if (input.length < 3) {
          return resolve([]);
        }
    ...
    getResultValue: function getResultValue(result) {
      return "[".concat(result.nis, "] ").concat(result.nama_lengkap, " ").concat(result.kelas.nama_kelas);
    },
    ...
    getTagihan: function getTagihan(id) {
      var _this = this;

      axios__WEBPACK_IMPORTED_MODULE_1___default.a.get("../getTagihan/".concat(id)).then(function (response) {
        // console.log(response.data)
        _this.tagihan = response.data;
      });
    },
    addToCart: function addToCart(id, harga, jenis, keterangan, tipe) {
      var _this2 = this;
      ...
      this.tagihan_id = id;
      axios__WEBPACK_IMPORTED_MODULE_1___default.a.get("../cart/addCart?id=".concat(id, "&jenis=").concat(jenis, "&harga=").concat(harga, "&keterangan=").concat(keterangan)).then(function (res) {
        Fire.$emit('afterAddCart'); // console.log(res)

        _this2.submitCart = false;
      })["catch"](function (err) {
        console.log(err);
      });
    },
    getCart: function getCart() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_1___default.a.get("../cart").then(function (res) {
        _this3.cart = res.data.cartItems;
        _this3.cartTotal = res.data.cartTotal; // console.log(this.cartTotal)
      })["catch"](function (err) {
        console.log(err);
      });
    },
    deleteItem: function deleteItem(rowId, id) {
      var _this4 = this;

      console.log(id);
      this.submitCart = true;
      axios__WEBPACK_IMPORTED_MODULE_1___default.a["delete"]("../cart/deleteItem/".concat(rowId)).then(function (res) {
        // console.log(res)
        Fire.$emit('afterAddCart');
        _this4.submitCart = false;
        _this4.submit[id] = false;
      })["catch"](function (err) {
        console.log(err);
      });
    },
    removeAllCart: function removeAllCart() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_1___default.a.get("../cart/removeCart").then(function (response) {
    ...
    bayarAngsuran: function bayarAngsuran(id, harga, jenis, keterangan) {
    ...
    bayarAngsuranSubmit: function bayarAngsuranSubmit() {
      if (parseInt(this.tmp.harga) <= 10000) {
        this.showError = true;
        return;
      }
    ...
    storePembayaran: function storePembayaran(siswaId) {
      var _this6 = this;

      console.log('store pembayaran');
      this.simpanBtn = true; // url = 'http://localhost:8000/'

      axios__WEBPACK_IMPORTED_MODULE_1___default.a.get("../cartSimpan?siswaId=".concat(siswaId)).then(function (res) {
        // console.log(res)
        Fire.$emit('afterAddCart');
        _this6.showModal = false;
        _this6.simpanBtn = false;
        iziToast.success({
          title: '',
          message: 'Pembayaran Berhasil!',
          position: 'bottomCenter'
        });
        _this6.submit[_this6.tmp.id] = false;

        _this6.getTagihan(siswaId);
      })["catch"](function (err) {
        console.log(err);
      });
    }
...
/***/ 1:
/*!******************************************!*\
  !*** multi ./resources/js/pembayaran.js ***!
  \******************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\game\xampp\htdocs\lara7-spp\resources\js\pembayaran.js */"./resources/js/pembayaran.js");
